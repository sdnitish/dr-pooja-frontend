 ...
 * - uploads files to /api/upload?folder=website (if changed)
...
export default function AdminSettingsPage() {
  ...
  // upload single File to /api/upload?folder=... -> returns { filename, url, folder }
  async function uploadFile(file, folder = "website") {
    if (!file) return null;
    const fd = new FormData();
    fd.append("file", file);

    // tell the new generic upload route which folder to use
    ...
    const data = await res.json();
    // data = { filename: "xxx.png", url: "/images/website/xxx.png", folder: "website" }
    return data;
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (logoFile) {
        // explicitly upload to website folder
        const uploaded = await uploadFile(logoFile, "website"); // returns { filename, url }
        logoFilename = uploaded.filename;
      }

      if (faviconFile) {
        const uploaded = await uploadFile(faviconFile, "website");
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Recommended: square small PNG — will be stored in public/images/website</p>
              </div>
            </div>
          </div>
        </div>
